...
use zebra_state::{ReadRequest, ReadResponse, TachyonMiningData};
...
fn valid_for_candidate(
    ...
    for transaction in transactions {
        ...
        let Some(data) = transaction.transaction.transaction.tachyon_shielded_data() else {
...
mod tests {
    ...
    fn assert_aggregated(
        ...
        original_ids: &[zebra_chain::transaction::Hash],
    ) {
        assert_eq!(
            aggregated
                .iter()
    ...
    fn verified_dependent_transaction(parent: zebra_chain::transaction::Hash) -> VerifiedUnminedTx {
        let transaction = Transaction::V7 {
            network_upgrade: NetworkUpgrade::ZFuture,
            lock_time: LockTime::min_lock_time_timestamp(),
            expiry_height: Height(0),
