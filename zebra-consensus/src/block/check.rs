...
/// proof of their own but carry an explicit `wtxid` reference (assigned by
/// the block producer) identifying which stamped transaction in the block
/// covers their actions.
///
/// This function:
/// 1. Walks the block once to collect stamped bundles, keyed by their
///    transaction's wtxid.
/// 2. Walks the block again to attribute each stripped bundle's actions to
///    the stamp whose wtxid it references.
/// 3. Verifies each stamp's proof against the combined actions.
#[cfg(all(zcash_unstable = "nu7", feature = "tx_v6"))]
pub fn verify_tachyon_aggregates(block: &Block) -> Result<(), BlockError> {
    use std::collections::HashMap;

    use zcash_tachyon::TachyonBundle;
    use zebra_chain::transaction::{TachyonShieldedData, WtxId};

    let mut groups: HashMap<[u8; 64], (&zcash_tachyon::Stamp, Vec<zcash_tachyon::Action>)> =
        HashMap::new();

    for tx in &block.transactions {
        if let Transaction::V6 {
            tachyon_shielded_data: Some(TachyonShieldedData(TachyonBundle::Stamped(bundle))),
            ..
        } = tx.as_ref()
        {
            let wtxid = WtxId::from(tx.as_ref()).as_bytes();
            if groups
                .insert(wtxid, (&bundle.stamp, bundle.actions.clone()))
                .is_some()
            {
                return Err(BlockError::Other(
                    "duplicate tachyon aggregate wtxid in block".to_string(),
                ));
            }
        }
    }
